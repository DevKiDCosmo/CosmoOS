...
typedef struct {
  ...
  uint8_t VolumeLabel[11]; // 11 bytes long, Padded with space
  uint8_t SystemId[8];
...
bool readFile(DirectoryEntry* fileEntry, FILE* disk, uint8_t* outputBuffer) {
  ...
  do {
    ...
    if(currentCluster % 2 == 0) {
      currentCluster = (*(uint16_t*)(g_Fat + fatIndex)) & 0x0FFF;  //g_Fat[fatIndex] | (g_Fat[fatIndex + 1] & 0x0F) << 8;
    } else {
      currentCluster = (*(uint16_t*)(g_Fat + fatIndex)) >> 4; // (g_Fat[fatIndex] >> 4) | (g_Fat[fatIndex + 1] << 4);
    }
